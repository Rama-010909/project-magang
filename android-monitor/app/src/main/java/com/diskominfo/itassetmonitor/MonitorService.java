...
import android.app.*;import android.content.*;import android.os.*;import java.io.*;import java.net.*;import java.nio.charset.StandardCharsets;import java.text.*;import java.util.*;import org.json.*;

public class MonitorService extends Service {
  static final String PROJECT="it-asset-diskominfo-batang"; static final String API_KEY="AIzaSyCnybMKpM7Z5gWn49hIsd5ymhFVSVtEuoo"; String idToken=null; long tokenAt=0;
  ...
  void ensureAuth()throws Exception{ if(idToken!=null && System.currentTimeMillis()-tokenAt<45*60*1000)return; String raw=requestRaw("https://identitytoolkit.googleapis.com/v1/accounts:signUp?key="+URLEncoder.encode(API_KEY,"UTF-8"),"POST","{\"returnSecureToken\":true}",false); JSONObject j=new JSONObject(raw); idToken=j.optString("idToken",""); if(idToken.isEmpty())throw new IOException("Anonymous Auth gagal"); tokenAt=System.currentTimeMillis(); }
  ...
  String requestRaw(String u,String method,String body,boolean auth)throws Exception{HttpURLConnection c=(HttpURLConnection)new URL(u).openConnection();c.setConnectTimeout(5000);c.setReadTimeout(5000);c.setRequestMethod(method);c.setRequestProperty("Content-Type","application/json");if(auth && idToken!=null)c.setRequestProperty("Authorization","Bearer "+idToken);if(body!=null){c.setDoOutput(true);try(OutputStream o=c.getOutputStream()){o.write(body.getBytes(StandardCharsets.UTF_8));}}int r=c.getResponseCode();InputStream in=r>=400?c.getErrorStream():c.getInputStream();String s="";if(in!=null){try(BufferedReader br=new BufferedReader(new InputStreamReader(in))){String x;StringBuilder z=new StringBuilder();while((x=br.readLine())!=null)z.append(x);s=z.toString();}}if(r>=400)throw new IOException("HTTP "+r+" "+s);return s;}
  ...
  @Override public int onStartCommand(Intent i,int f,int id){return START_STICKY;} @Override public IBinder onBind(Intent i){return null;} @Override public void onDestroy(){if(loop!=null)h.removeCallbacks(loop);super.onDestroy();}
}
